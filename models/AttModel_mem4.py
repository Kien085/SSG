...
class AttModel_mem4(CaptionModel):
    ...
    def _forward(self, seq, ssg_data=None, use_rela =0, training_mode = 0):
        """
        ...
        :return:
        """
        #I wonder if I can trust this documentation, and just do training mode 0 and 1
        #If i do this, then I don't have to do anything having to prune the code and what not.
        #Instead, i can just train with training_mode 0 and 1, and never go to training_mode 2

        #att_feats, att_masks = self.clip_att(att_feats, att_masks)
        self.index_eval = 0
        batch_size = self.batch_size #fc_feats.size(0)
        ...
        for i in range(seq.size(1) - 1):
            ...
            output, state = self.get_logprobs_state(
                it, att_feats_mem, p_att_feats_mem, att_feats_rela,
    ...
    def get_logprobs_state(self, it, att_feats_mem, p_att_feats_mem, att_feats_rela,
                           p_att_feats_rela, att_masks, training_mode, state):
        ...
        if training_mode == 0 or training_mode == 1:
            output, state = self.ssg_core(xt, att_feats_mem, p_att_feats_mem, state, att_masks)
    ...
    def _sample(self, fc_feats, att_feats, att_masks=None, rela_data=None, ssg_data=None, use_rela = 1, training_mode=0, opt={}):
        ...
        #We are not doing training_mode 2
        # if training_mode == 2:
        #     if use_rela == 1:
        #         rela_data['att_feats'] = att_feats
        #         rela_data['att_masks'] = att_masks
        ...
        #A lot of things are based off of fc_feats
        #I wonder if there is another way of doing this
        #Most of it is new_zeros, so I just need to know another tensor that has the same type
...
class TopDownCore_mem(nn.Module):
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks): #why do we need all of these things for ssg
        prev_h = state[0][-1]
